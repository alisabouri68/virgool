...
lis.addEventListener('mouseout',()=>{
  ...
  if(datasetc == a){
   ...
   arrsub.map((val)=>{
    ...
    li.onclick=liDataGet
    li.innerHTML = `
    <a href="${window.location.pathname==="/index.html"?"assets/pages/category.html":"./category.html"}" data-content="${val}" data-bcontent="${datasetc}" class=" w-full h-full"><span class="text-sm pointer-events-none">${val}</span></a>
    `;
...
console.log(headerContainer.innerWidth
)
